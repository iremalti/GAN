...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        # Insgesamt kann man sagen, dass die Schichten in self.features in der Regel für die Merkmalsextraktion und
        # die Schichten in self.main für die Klassifizierung zuständig sind.
        self.main = nn.Sequential(
...
# Initialize the models and optimizers
    ...
    for epoch in range (EPOCH_NUM):
        for i, data in enumerate(dataloader):
            ...
            if i % 100 == 0:
                ...
                vutils.save_image(x_real, os.path.join(OUT_PATH, 'real_sample_zahl.png'), normalize = True)
                with torch.no_grad():
                    viz_sample = netG(viz_noise)
                    vutils.save_image(viz_sample, os.path.join(OUT_PATH, 'fake_sample_zahl_{}.png'.format(epoch)),normalize=True)
